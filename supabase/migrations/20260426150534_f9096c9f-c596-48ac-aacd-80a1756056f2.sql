...
BEGIN
  ...
  IF v_count >= v_tournament.total_slots THEN RAISE EXCEPTION 'Slots full'; END IF;

  IF v_tournament.entry_fee > 0 THEN
    IF v_profile.coins < v_tournament.entry_fee THEN RAISE EXCEPTION 'Insufficient coins'; END IF;
...
BEGIN
  ...
  SELECT * INTO v_profile FROM public.profiles WHERE id = v_user FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Player profile not found'; END IF;
  IF v_profile.coins < _amount THEN RAISE EXCEPTION 'Insufficient coins'; END IF;
